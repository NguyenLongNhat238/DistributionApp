...
class UserInformationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'first_name', 'last_name', 'username', 'birthday', 'sex', 'email',
                  'phone', 'avatar', 'identity_card', 'city', 'district', 'ward', 'street']
        extra_kwargs = {
            'username': {
                'read_only': True
            },
...
class ChangePasswordUserSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if user_ok is None:
            raise serializers.ValidationError(
                {'error': "Mật khẩu cũ không hợp lệ"})

        error = custom_password_validation.validate_password(
            password=password)
