...
from constant.config import HOST, CLIENT
...
def check_valid_otp(otp_code, token):
    ...
    try:
        ...
        if payload['random_code'] != otp_code:
            raise exceptions.AuthenticationFailed(
                ErrorHandling(message="Mã xác thực không hợp lệ / Your OTP code is incorrect", code="DATA INVALID", type="DATA INVALID", lang="vi").to_representation())
...
def send_email_password_change(user):
    ...
    try:
        ...
        msg_html = render_to_string('acc_password_change.html', {
        ...
        raise exceptions.server_error(
            ErrorHandling(message="Mã token đã hết hạn / Your token has expired",
                          code="SERVER_DOWN", type="SERVER_DOWN", lang="vi").to_representation()
        )
...
def check_password_forgot_core(uidb64, token, password):
    ...
    try:
        ...
        raise exceptions.AuthenticationFailed(
            ErrorHandling(message="Không thể giải mã uidb64 / Unable to decode uidb64",
                          code="DATA INVALID", type="DATA INVALID", lang="vi").to_representation()
        )

    if user_id_uidb64 != user_id_token:
        raise exceptions.AuthenticationFailed(
            ErrorHandling(message="uidb64 không hợp lệ / uidb64 invalid",
                          code="DATA INVALID", type="DATA INVALID", lang="vi").to_representation()
        )
    ...
    try:
        ...
        raise exceptions.AuthenticationFailed(
            ErrorHandling(message="Email không tồn tại / Your email has not existed",
                          code="DATA INVALID", type="DATA INVALID", lang="vi").to_representation()
        )
