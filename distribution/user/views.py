...
from .serializers import LoginSerializer, SignupSerializer, UserInformationSerializer, UserListSerializer, \
    ChangePasswordUserSerializer
from .services import gen_otp_and_token, check_valid_otp, send_email_password_forgot, \
    check_password_forgot_core, send_email_password_change
...
class UserViewSet(viewsets.ViewSet):
            ...
            if not user.is_active:
                ...
                return Response(
                    ErrorHandling(
                        ...
                        code="FAILURE", type="FAILURE", lang="vi",
                        token_activation=token_activation).to_representation(), status=status.HTTP_401_UNAUTHORIZED)

            serializer = LoginSerializer(user)
            user.last_login = datetime.datetime.now()
            user.save()
            return Response(SuccessHandling(
                message="Đăng nhập thành công / Login successfully", code="SUCCESS", type="SUCCESS", lang="vi",
                **serializer.data).to_representation(), status=status.HTTP_200_OK)

        return Response(
            ErrorHandling(message="Sai email hoặc mật khẩu / Your email or password is wrong", code="DATA INVALID",
                          type="DATA INVALID", lang="vi").to_representation(), status=status.HTTP_200_OK)
    ...
    def signup(self, request):
        ...
        if serializer.is_valid():
            ...
            if phone is not None and email is not None:
                ...
                # return token_activatation
                return Response(SuccessHandling(message="Bạn đã đăng ký thành công / Your registration is successfully",
                                                code="SUCCESS", type="SUCCESS", lang="vi",
                                                token_activation=token_activation).to_representation(),
                                status=status.HTTP_200_OK)
            else:
                return Response(ErrorHandling(message="register by phone is not supported", code="DATA INVALID",
                                              type="DATA INVALID", lang="vi").to_representation(),
                                status=status.HTTP_400_BAD_REQUEST)

        error_message = [serializer.errors[error_name][0]
                         for error_name in serializer.errors][0]

        return Response(ErrorHandling(message=error_message, code="DATA INVALID", type="DATA INVALID",
                                      lang="vi").to_representation(), status=status.HTTP_400_BAD_REQUEST)
    ...
    def verify(self, request):
        ...
        if is_valid == True:
            try:
                ...
                user.save()
                return Response(
                    SuccessHandling(message="Xác thực của bạn đã thành công / Your verification is successfully",
                                    code="SUCCESS", type="SUCCESS", lang="vi").to_representation(),
                    status=status.HTTP_200_OK)
            except:
                return Response(ErrorHandling(message="Địa chỉ email không tồn tại / The email has not existed",
                                              code="DATA INVALID", type="DATA INVALID", lang="vi").to_representation(),
                                status=status.HTTP_400_BAD_REQUEST)
        else:
            return Response(ErrorHandling(message="500 Internal Server Error", code="SERVER_DOWN", type="SERVER_DOWN",
                                          lang="vi").to_representation(), status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def send_otp_code(self, request):
        ...
        if user:
            if user.is_active == False:
                ...
                return Response(ErrorHandling(message="You are already active", code="DATA INVALID",
                                              type="DATA INVALID", lang="vi").to_representation(),
                                status=status.HTTP_400_BAD_REQUEST)

        return Response(ErrorHandling(message='Email ERROR', code="DATA INVALID", type="DATA INVALID",
                                      lang="vi").to_representation(), status=status.HTTP_400_BAD_REQUEST)
    ...
    def send_password_forgot(self, request):
        ...
        try:
            ...
            return Response(
                ErrorHandling(message="Địa chỉ email không tồn tại / The email has not existed", code="DATA INVALID",
                              type="DATA INVALID", lang="vi").to_representation(), status=status.HTTP_400_BAD_REQUEST)

        send_email_password_forgot_task.delay(user_id=user.id)
        return Response(
            SuccessHandling(message="Email đặt lại mã pin đã gửi đến hộp thư của bạn", code="SUCCESS", type="SUCCESS",
                            lang="vi").to_representation(), status=status.HTTP_200_OK)
    ...
    def password(self, request):
        ...
        return Response(
            SuccessHandling(message="Thay đổi mật khẩu thành công / Change your password successfully", code="SUCCESS",
                            type="SUCCESS", lang="vi").to_representation(), status=status.HTTP_200_OK)


class UserInformationViewSet(viewsets.ViewSet, generics.UpdateAPIView, generics.ListAPIView):
    ...
    def change_password(self, request, pk):
        ...
        return Response(
            SuccessHandling(
                ...
                code='SUCCESS', type='SUCCESS', lang='en').to_representation(), status=status.HTTP_200_OK)


class UserRoleViewSet(viewsets.ViewSet):
    def list(self, request, *args, **kwargs):
        data = [{
            "name": User.ROLE_SHIPPER
        }, {
            "name": User.ROLE_SYSTEM_ADDMIN
        }, {
            "name": User.NONE
        }]
