...
class UserViewSet(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        ...
        responses={
            200: "Đăng ký thành công / Your registration is successfully",
        },
    )
    @action(detail=False, methods=["post"])
...
class UserInformationViewSet(
    viewsets.ViewSet, generics.UpdateAPIView, generics.ListAPIView
):
...
class UserGenderViewSet(viewsets.ViewSet):
    def list(self, request, *args, **kwargs):
        data = [{"name": User.SEX_FEMALE}, {"name": User.SEX_MALE}, {"name": User.NONE}]
        return Response(data=data, status=status.HTTP_200_OK)
